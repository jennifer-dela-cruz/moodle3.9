...
if ($page == -1) {
    ...
    $nexturl = $attemptobj->attempt_snap_proctor_url(null, $page, $proctor_type_page);
    if ($scrollpos !== '') {
...
if (!$finishattempt && !$attemptobj->check_page_access($thispage)) {
    throw new moodle_exception('submissionoutofsequencefriendlymessage', 'question',
            $attemptobj->attempt_snap_proctor_url(null, $attemptobj->get_currentpage()));
}
// echo 'before processing the attempt';

// Process the attempt, getting the new status for the attempt.
$status = $attemptobj->process_attempt_snap_proctor($timenow, $finishattempt, $timeup, $thispage);
